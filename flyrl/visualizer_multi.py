...
class DogfightVisualizer:
    ...
    def __init__(self, max_history_points: int = 100, update_interval: int = 100, 
                 show_waypoints: bool = True, show_waypoint_line: bool = True):
        """
        ...
        Args:
            ...
            show_waypoints: Whether to show enemy waypoints
            show_waypoint_line: Whether to show line from enemy to waypoint
        """
        self.max_history_points = max_history_points
        self.update_interval = update_interval
        self.show_waypoints = show_waypoints
        self.show_waypoint_line = show_waypoint_line
        ...
        self.enemy_waypoint_history = []  # Track waypoint changes
        ...
        # Waypoint visualization elements
        self.enemy_waypoint_marker = None
        self.enemy_waypoint_line = None
    ...
    def _setup_plot(self):
        ...
        if self.show_waypoints:
            ...
            if self.show_waypoint_line:
                self.enemy_waypoint_line, = self.ax.plot([], [], [], 'g--', linewidth=1, 
                                                        alpha=0.6, label='To Waypoint')
    ...
    def update_positions(self, own_pos: Tuple[float, float, float], 
                        ...
                        enemy_waypoint: Tuple[float, float, float] = None):
        """
        ...
        if len(self.enemy_waypoint_history) > self.max_history_points:
            self.enemy_waypoint_history.pop(0)
    ...
    def update_from_simulation(self, env):
        ...
        Args:
            env: Environment object with simulation data
        """
        ...
        enemy_lat = env.enemy_sim[prp.lat_geod_deg]
        enemy_lon = env.enemy_sim[prp.lng_geoc_deg]
        enemy_alt = env.enemy_sim[prp.altitude_sl_ft] * 0.3048
    ...
    def _calculate_engagement_metrics(self) -> Dict:
        ...
        if self.current_enemy_waypoint:
            ...
            metrics.update({
                ...
                'waypoint_alt': waypoint_alt,
                'waypoint_bearing': math.atan2(waypoint_pos[1] - enemy_pos[1], 
                                             waypoint_pos[0] - enemy_pos[0]) * 180 / math.pi
            })
    ...
    def clear_trajectories(self):
        ...
        self.enemy_waypoint_history.clear()
        self.current_enemy_waypoint = None
...
def example_usage():
    ...
    # Create visualizer with waypoint visualization enabled
    visualizer = DogfightVisualizer(max_history_points=200, update_interval=50,
                                  show_waypoints=True, show_waypoint_line=True)
    ...
    def update_visualization(dogfight_task):
        ...
        # Update visualizer
        visualizer.update_from_simulation(dogfight_task)
