...
class MultiAircraftDogfightTask(MultiAircraftFlightTask):
    ...
    def calculate_reward(self, state, done) -> float:
        ...
        roll_rate_penalty = -max(0, abs(self.get_player_prop(prp.p_radps)) - 1.0) * 0.5 # 1 rad/s
        pitch_rate_penalty = -max(0, abs(self.get_player_prop(prp.q_radps)) - 0.8) * 0.5  # 0.8 rad/s
        
        step_reward = (distance_reward + 2.0 * heading_reward + elevation_reward + 
                    lock_bonus + step_penalty + 
